...
fun MovieDetailContent(
    ...
    Column(modifier = mainModifier) {
        ...
        Spacer(modifier = Modifier.height(4.dp))
        Text(
            text = DurationTime(runtime = movie.runtime),
            style = Fonts.Typography.body1.copy(color = Color.Gray)
        )

        // region genres
        Header(title = stringResource(id = R.string.genres))
        MovieGenreContent(genres = movie.genres)
        // endregion

        // region descriptions
        Header(title = stringResource(id = R.string.description))
        Text(
            text = movie.overview ?: "",
            style = Fonts.Typography.body1.copy(color = Color.Gray)
        )
...
@Composable
fun DurationTime(runtime: Int): String {
    var time = "${runtime / 60} ${stringResource(id = R.string.hours)}"
    if (runtime % 60 > 0) time += " ${runtime % 60} ${stringResource(id = R.string.minutes)}"
    return time
}
